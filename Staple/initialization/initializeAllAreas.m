function [params, bg_area, fg_area, area_resize_factor] = initializeAllAreas(im, params)
    ...
    %bg_area:背景区域；fg_area:前景区域

	% we want a regular(定期；常规) frame surrounding the object
	avg_dim = sum(params.target_sz)/2;
	% size from which we extract features
	bg_area = round(params.target_sz + avg_dim);%背景区域
	% pick a "safe" region smaller than bbox to avoid mislabeling
	fg_area = round(params.target_sz - avg_dim * params.inner_padding);%前景区域
	% saturate(饱和) to image size
	if(bg_area(2)>size(im,2)), bg_area(2)=size(im,2)-1; end
	if(bg_area(1)>size(im,1)), bg_area(1)=size(im,1)-1; end
	% make sure the differences are a multiple of 2(确保差异是2的倍数) (makes things easier later in color histograms)
    %differences,即bg_area - params.target_sz 是2的倍数，bg_area - fg_area也是2的倍数
	...
	area_resize_factor = sqrt(params.fixed_area/prod(bg_area));%将背景区域统一设为fixed_area大小，文中说这样是为了speed.
	...
	params.cf_response_size = floor(params.norm_bg_area / params.hog_cell_size);%params中设置的cell_size=4
	% given the norm BG area, which is the corresponding target w and h?
    %经过测试，这样使用0.75与0.25得到的norm_target_sz_w，norm_target_sz_h与下面注释掉的使用
    %params.norm_bg_area(2)/ bg_area(2)得到的数值几乎没有差别，但是速度会快。
%     tic();
...
%     toc()
	...
	norm_pad = floor((params.norm_bg_area - params.norm_target_sz) / 2);%padding区域大小，在四周进行padding
	radius = min(norm_pad);
	...
	params.norm_pwp_search_area = params.norm_target_sz + params.norm_delta_area - 1;%将最终区域面积转化到接近或等于150^2。
