...
% RUN_TRACKER  is the *external* function of the tracker - does configuration and calls THISTRACKER
    ...
    videoFrame = snapshot(params.myWebcam);%snapshot - 从摄像头捕获单一图像帧
    ...
    BBOXW = 200;
    BBOXH = 280;
    BBOXX = (frameSize(2) - BBOXW)/2;%目标框左上角x坐标
...
%             get(h, 'Visible') % will return 'off' if the figure is not visible.
    ...
    while ~STOPTRACKING
        % pad = round(rand() * 100);
        pad = 0;
        diff = pad - prevpad;%这里pad与prevpad一直是0
        rect = rect + diff;

        im = snapshot(params.myWebcam);        
        im=padarray(im,[pad pad], NaN);%This MATLAB function pads array A with 0s (zeros).
