...
v_real_pos = 1/sqrt(2) * ones(length(dft_pos_ind), 1);%论文中式(7)公式
...
dfs_matrix = sparse(i_tot, j_tot, v_tot, dft_length, dft_length);%sparse - 创建稀疏矩阵,此 MATLAB 函数 通过挤出任何零元素将满矩阵转换为稀疏格式。如果矩阵包含许多零，将矩阵转换为稀疏存储空间可以节省内存。