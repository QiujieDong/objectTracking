% Partitions(分区) the spectrum(光谱) of a 2-dimensional signal with dimensions dft_sz into
...
spec_dim = ceil((dft_sz+1)/2);%dft_sz为目标区域的height与width
dim_even = mod(dft_sz, 2) == 0;
if dim_even(2) %如果dft_sz的width也就是use_sz的width正好被2整除
    % linear indecies of the part of the spectrum that is needed [g_0, g_+]
    dft_ind = [1:spec_dim(1), (dft_sz(1)+1):((spec_dim(2)-1)*dft_sz(1)+spec_dim(1))]';%冒号的运算优先级是最低的,dft_int=[1:26,51:1276]'的向量
    ...
    dft_pos_ind = [2:spec_dim(1)-dim_even(1), dft_sz(1)+1:(spec_dim(2)-1)*dft_sz(1), (spec_dim(2)-1)*dft_sz(1)+(2:spec_dim(1)-dim_even(1))]';
                                                                                  %dft_pos_ind=[2:25,51:1250,1252:1275]'的向量
else
...
dft_sym_ind = setdiff(dft_ind, dft_pos_ind);%setdiff - 设置两个数组的差集.此 MATLAB 函数 返回 A 中存在但 B 中不存在的数据，不包含重复项。
                                            %dft_sym_ind=[1,26,1251,1276]'的向量
...
[dft_neg_ind_unsorted, ~, corresponding_dft_pos_id] = find(neg_dft_loc(:)); % find the indices and corresponding positive id:s%find - 查找非零元素的索引和值dft_neg_ind_unsorted为非零元素行，corresponding_dft_pos_id为非零元素值
[~, dft_neg_ind_order] = sort(corresponding_dft_pos_id);    % sort the id:s
dft_neg_ind = dft_neg_ind_unsorted(dft_neg_ind_order);      % get the corresopnding negative indices,dft_neg_ind=[27:50,1277:2500]'的向量