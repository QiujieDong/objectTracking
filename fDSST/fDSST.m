...
if prod(init_target_sz) > translation_model_max_area
    currentScaleFactor = sqrt(prod(init_target_sz) / translation_model_max_area);%若target_sz大于了translation_model_max_area，则算出比例，下面第36行处理
else
    currentScaleFactor = 1.0;
end

% target size at the initial scale
base_target_sz = target_sz / currentScaleFactor;%target_sz大于了translation_model_max_area，则target_sz为translation_model_max_area大小

%window size, taking padding into account
sz = floor( base_target_sz * (1 + padding ));

featureRatio = 4;%特征比率，就是HOG里的cell_size

output_sigma = sqrt(prod(floor(base_target_sz/featureRatio))) * output_sigma_factor;%这个式子和KCF的式子其实是一样的
use_sz = floor(sz/featureRatio);
rg = circshift(-floor((use_sz(1)-1)/2):ceil((use_sz(1)-1)/2), [0 -floor((use_sz(1)-1)/2)]);%circshift循环移位，向左平移floor((use_sz(1)-1)/2)，将中心移到开头
cg = circshift(-floor((use_sz(2)-1)/2):ceil((use_sz(2)-1)/2), [0 -floor((use_sz(2)-1)/2)]);

[rs, cs] = ndgrid( rg,cg);
y = exp(-0.5 * (((rs.^2 + cs.^2) / output_sigma^2)));%低频中心移到了左上角
yf = single(fft2(y));
...
if nScales > 0
    ...
    scale_exp = (-floor((nScales-1)/2):ceil((nScales-1)/2)) * nScalesInterp/nScales;%* nScalesInterp/nScales将尺度池由(-8,8)放大到接近(-16,16)
    scale_exp_shift = circshift(scale_exp, [0 -floor((nScales-1)/2)]);%与上面rg,cg对应的移动
    ...
    max_scale_dim = strcmp(params.s_num_compressed_dim,'MAX');%strcmp - 比较字符串,如果二者相同，则返回 1 (true)，否则返回 0
    if max_scale_dim
...
for frame = 1:num_frames
    ...
    if frame > 1
        old_pos = inf(size(pos));%inf(row,column) - 无穷大.此 MATLAB 函数返回正无穷大的矩阵，维度为row行column列
        iter = 1;
        ...
        while iter <= refinement_iterations && any(old_pos ~= pos)
            ...
            % response so it has the same size as the image patch 如果我们对功能进行欠采样，我们希望插入响应，使其与图像补丁具有相同的大小
            if interpolate_response > 0
                if interpolate_response == 2
                    % use dynamic interp size 使用动态interp大小
            ...
            switch interpolate_response
                case 0
                    translation_vec = round([disp_row, disp_col] * featureRatio * currentScaleFactor);%round - 四舍五入为最近的小数或整数
                case 1
    ...
    [xl_npca, xl_pca] = get_subwindow(im, pos, sz, currentScaleFactor);%这里x1_npca为0x0矩阵
    ...
    hf_proj = fft2(feature_projection(h_num_npca, h_num_pca, projection_matrix, cos_window));%feature_projection加窗处理
    ...
    if nScales > 0
        ...
        if max_scale_dim
            [scale_basis, ~] = qr(bigY, 0);%qr - 正交三角分解.此 MATLAB 函数（其中 A 是 m×n）生成 m×n 上三角形矩阵 R 和 m×m 酉矩阵 Q，这样 A = Q*R。
            [scale_basis_den, ~] = qr(bigY_den, 0);
        else
            [U,~,~] = svd(bigY,'econ');%奇异值分解
