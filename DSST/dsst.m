...
output_sigma = sqrt(prod(base_target_sz)) * output_sigma_factor;%labe的高斯带宽
[rs, cs] = ndgrid((1:sz(1)) - floor(sz(1)/2), (1:sz(2)) - floor(sz(2)/2));
y = exp(-0.5 * (((rs.^2 + cs.^2) / output_sigma^2)));
yf = single(fft2(y));%单精度数组


% desired scale filter output (gaussian shaped), bandwidth proportional to
% number of scales
scale_sigma = nScales/sqrt(33) * scale_sigma_factor;
ss = (1:nScales) - ceil(nScales/2);%ceil - 朝正无穷大四舍五入
...
if mod(nScales,2) == 0 %mod-模除
    scale_window = single(hann(nScales+1)); % 保证n_Scales为奇数，这样在base_target_sz两边的数量就会相等。
    scale_window = scale_window(2:end); %截取数量保证nScales维度一致，上面已经对scale_window加完窗了。
else
...
% scale factors
ss = 1:nScales;
scaleFactors = scale_step.^(ceil(nScales/2) - ss);% 尺度金字塔，向下取整scale_step^(-(s-1)/2,……,(s-1)/2)

% compute the resize dimensions used for feature extraction in the scale
% estimation
scale_model_factor = 1;
if prod(init_target_sz) > scale_model_max_area %像素区域设置
    scale_model_factor = sqrt(scale_model_max_area/prod(init_target_sz));%对于初始面积大于512pix的目标，计算保留纵横比的512pix面积的固定大小，
                                                                         %这确定了最大特征描述符长度为992
end
scale_model_sz = floor(init_target_sz * scale_model_factor);%像素搜索区域
...
min_scale_factor = scale_step ^ ceil(log(max(5  ./ sz)) / log(scale_step));%sz为搜索区域，限制区域，防止出区域问题
...
for frame = 1:num_frames
    ...
    if frame > 1
        ...
        % update the position
        pos = pos + round((-sz/2 + [row, col]) * currentScaleFactor); %round - 四舍五入为最近的小数或整数
    ...
    xl = get_translation_sample(im, pos, sz, currentScaleFactor, cos_window);%输出为经过加窗处理的当前帧图像patch的HOG矩阵
    
    % calculate the translation filter update
    xlf = fft2(xl);
    new_hf_num = bsxfun(@times, yf, conj(xlf));%@time-数组乘法
    new_hf_den = sum(xlf .* conj(xlf), 3);% S = sum(A,dim) 沿维度 dim 返回总和。
