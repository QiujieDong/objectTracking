...
% 		end
		...
		if frame > 1
			...
			[vert_delta,tmp, horiz_delta] = find(response == max(response(:)), 1);%按顺序第一个最大值的位置；vert_delta是行，tmp是列(这里的列是在整个矩阵上的列)
                                                                               % horiz_delta是取得的数组成的向量，这里response == max(response(:))生成的三位矩阵已经是0,1矩阵，最大值为1.

            szid = floor((tmp-1)/(size(cos_window,2)))+1;%取得最大值时的第三维，也就是尺度维。

            horiz_delta = tmp - ((szid -1)* size(cos_window,2));%取得在最大值层的列数
